...
"#,
    ...
    let mut child = cmd
        // Close stdin: `codex exec` otherwise reads stdin in addition to the
        // prompt arg and blocks ("Reading additional input from stdin…") until
        // EOF, hanging the whole run when our stdin is an open pipe/TTY.
        .stdin(Stdio::null())
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
